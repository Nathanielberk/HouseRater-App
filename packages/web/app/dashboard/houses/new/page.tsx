...
export default function NewHousePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: house, error: insertError } = await supabase
        .from('houses')
        .insert([houseData])
        .select()
        .single()
      ...
      if (insertError) {
        console.error('Error creating house:', insertError)
        // Show more specific error message
        const errorMessage = insertError.message || 'Unknown error'
        if (errorMessage.includes('is_active')) {
          setError('Database error: Missing required column. Please contact support.')
        } else if (errorMessage.includes('duplicate')) {
          setError('This house already exists in your list.')
        } else {
          setError(`Failed to create house: ${errorMessage}`)
        }
        setLoading(false)
        return
      }
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-6 mb-6">
            ...
            <div className="mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              <p className="text-sm text-blue-800 dark:text-blue-200">
                <strong>Coming soon:</strong> Google Places address autocomplete will be enabled once you add an API key.
